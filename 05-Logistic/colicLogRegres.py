...
"""
示例：从疝气病症预测病马的死亡率
数据报告368个样本和28个特征，数据集包含了马疝病的一些指标，
有的指标比较主观，有的指标难以测量，例如马的疼痛级别。
除了部分指标主观和难以测量外，该数据还存在一个问题，
数据集中有30%的值是缺失的，首先要解决数据缺失问题，再用Logistic回归来预测病马的生死。
"""
...
def colicTest():
	...
	# trainWeights=trainWeights.flatten() #注意维度匹配
	# print(trainWeights.flatten())
...
def colicSklearn():
	...
	classifier = LogisticRegression(solver = 'sag',max_iter = 12).fit(trainingSet, trainingLabels)
